...
# =====================================================
# 3. FUNCIONES DE MODELOS (PROCESAMIENTO)
# =====================================================
...
def mostrar_resultados(df_final, df_agg, usar_colaborado, horizonte, fechas_dt, 
    ...
    # --- Nombres dinámicos para los meses ---
    ...
    for col in nombres_columnas_pron[:horizonte]:
        if col in df_filt.columns:
            proyeccion.append(df_filt[col].sum())
        else:
            proyeccion.append(0)
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if 'COD_ARTICULO' in cols_export:
            cols_export.remove('COD_ARTICULO')
            cols_export.insert(0, 'COD_ARTICULO')
        if 'DESCRIPCION' in cols_export:
            cols_export.remove('DESCRIPCION')
            cols_export.insert(1, 'DESCRIPCION')
        cols_export = [c for c in cols_export if c != 'GRUPO']
        df_agg_export = df_agg[cols_export]
        df_agg_export.to_excel(writer, sheet_name='Pronósticos (productos)', index=False)
    output.seek(0)
...
def procesar_archivo(archivo, rango_ventas, horizonte, usar_colaborado, col_colaborado):
    ...
    if usar_colaborado:
        try:
            ...
            st.error(f"No se pudo leer la columna {col_colaborado}. Error: {e}")
            usar_colaborado = False
            colaborado_series = None
    else:
        colaborado_series = None

    # Guardar serie histórica total para el gráfico
    ...
    for i in range(total):
        ...
        real_ultimo = float(serie_full.iloc[-1])
        real_ultimo = max(0, real_ultimo)

        if usar_colaborado and colaborado_series is not None:
            colab_val = colaborado_series.iloc[i]
            colab_val = max(0, float(colab_val))
        else:
            colab_val = None
        ...
        if mape_p <= mape_c:
            ...
            pronosticos = fit_prophet(serie_full, horizonte, reg_df)
        else:
        ...
        res = list(df_fijos.iloc[i]) + [
            real_ultimo,
            round(pron_ultimo, 2),
            round(mape_elegido * 100, 2),
            mejor_modelo,
        ] + [round(p, 2) for p in pronosticos]
        
        if usar_colaborado and colab_val is not None:
            mape_colab_linea = mean_absolute_percentage_error([real_ultimo + 1], [colab_val + 1]) * 100
            res.append(colab_val)
            res.append(round(mape_colab_linea, 2))

        resultados.append(res)

    # Crear fechas futuras para nombres de columnas
    ...
    if usar_colaborado:
        columnas_base.append('COLABORADO_ULTIMO')
        columnas_base.append('MAPE_COLABORADO_LINEA_%')

    df_final = pd.DataFrame(resultados, columns=columnas_base)

    # Agregación por producto y gerencia
    group_cols = ['COD_ARTICULO', 'GERENCIA']
    agg_dict = {
        'GRUPO': 'first',
        'ARTICULO_FAMILIA': 'first',
        'DESCRIPCION': 'first',
        'REAL_ULTIMO': 'sum',
        'PRON_ULTIMO': 'sum',
    }
    ...
    fin_total = time.time()
    duracion = (fin_total - start_time) / 60
    st.info(f"Tiempo total de procesamiento: {duracion:.2f} minutos")

    return df_final, df_agg, fechas_dt, hist_totales, nombres_columnas_pron, usar_colaborado

# =====================================================
# 6. INTERFAZ PRINCIPAL (LOGIN + PROYECTOS)
# =====================================================
...
with st.sidebar.form("nuevo_proyecto_form"):
    ...
    horizonte_nuevo = st.slider("Horizonte de pronóstico (meses)", min_value=1, max_value=12, value=12)
    ...
    if submit_nuevo:
        if not nombre_nuevo:
            st.error("Debe ingresar un nombre para el proyecto")
        elif not archivo_subido:
            st.error("Debe subir un archivo Excel")
        else:
            with st.spinner("Procesando archivo... Esto puede tomar varios minutos."):
                try:
                    df_final, df_agg, fechas_dt, hist_totales, nombres_columnas_pron, uc = procesar_archivo(
                    ...
                    guardar_proyecto(nombre_nuevo, df_final, df_agg, fechas_dt, uc, 
                                    horizonte_nuevo, nombres_columnas_pron, rango_ventas_nuevo, hist_totales)
...
if st.session_state.proyecto_actual:
    ...
    st.caption(f"Última modificación: {proyecto.get('fecha_creacion', 'No disponible')}")
    st.markdown("---")
    ...
    mostrar_resultados(
        proyecto['df_final'],
        proyecto['df_agg'],
        proyecto['usar_colaborado'],
        proyecto['horizonte'],
        proyecto['fechas_dt'],
        proyecto['hist_totales'],
    ...
    if proyectos:
        st.info(f"📁 Tienes {len(proyectos)} proyecto(s) guardado(s). Selecciona uno en el panel izquierdo para continuar.")
